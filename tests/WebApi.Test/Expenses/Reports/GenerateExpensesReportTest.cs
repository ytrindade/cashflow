...
public class GenerateExpensesReportTest : CashFlowClassFixture
{
    ...
    {
        var result = await DoGet($"{METHOD}/pdf?month={_expenseDate:yyyy-MM-dd}", _adminToken);
    ...
    {
        var result = await DoGet($"{METHOD}/excel?month={_expenseDate:yyyy-MM-dd}", _adminToken);
    ...
    {
        var result = await DoGet($"{METHOD}/pdf?month={_expenseDate:yyyy-MM-dd}", _teamMemberToken);
    ...
    {
        var result = await DoGet($"{METHOD}/excel?month={_expenseDate:yyyy-MM-dd}", _teamMemberToken);
